...
if ($cs.PartOfDomain) {
  Write-Log "NOTE: Instance already joined to domain ($($cs.Domain)). Exiting."
  exit 0
}

Write-Log "Install AD management components"
Install-WindowsFeature -Name GPMC,RSAT-AD-PowerShell,RSAT-AD-AdminCenter,RSAT-ADDS-Tools,RSAT-DNS-Server | Out-Null

Write-Log "Fetch domain join credentials"
...
Write-Log "Rebooting to finalize domain join"
